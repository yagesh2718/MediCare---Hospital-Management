    ...
    <div className="h-100 text-white px-4 py-12">
      ...
      <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6">
        {creditPlans.map((plan) => (
          <div
            key={plan.id}
            className="bg-emerald-900/30 border border-emerald-600/30 p-6 rounded-xl shadow hover:shadow-lg"
          >
            <h2 className="text-2xl font-semibold">{plan.name}</h2>
            <p className="text-zinc-400 mb-3">{plan.description}</p>
            <p className="text-4xl font-bold mb-2">₹{plan.price}</p>
            <p className="text-emerald-200 mb-4">{plan.credits} credits included</p>
            <button
              onClick={() => buyCredits(plan.id)}
              className="w-full bg-emerald-600 hover:bg-emerald-700 py-2 rounded text-white"
            >
              Buy Now
            </button>
          </div>
